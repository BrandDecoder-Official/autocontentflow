...
window.addAgentLog = async function(role, icon, message, isFinalSpinner = false, sourceEl = null) {
    try {
        ...
        if (targetDiv) {
            ...
            // ... 找到 targetDiv.innerHTML 那段 ...
            targetDiv.innerHTML = `
                <div class="text-2xl flex-shrink-0 bg-gray-700 p-2 rounded-lg shadow-sm self-start">${icon}</div>
                <div class="flex-1 min-w-0"> <div class="text-[11px] font-black text-gray-400 mb-1 tracking-wider uppercase">${role}</div>
                    <div class="text-sm font-bold text-gray-100 leading-relaxed break-words whitespace-normal"> ${message}${spinnerHtml}
                    </div>
                </div>
            `;
            logEl.scrollTop = logEl.scrollHeight; 
