...
window.executeWithRetry = async function(apiCallFn, role, actionName, maxRetries = 3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            await window.addAgentLog(role, '🛡️', `偵測到「${actionName}」擁塞，${Math.round(waitTime/1000)}秒後自動重試...`, true);
        ...
        callback: async function(response) {
            ...
            try {
                ...
                if (result.status === 'ACTIVE') {
                    ...
                    showToast(`✅ 登入成功！點數：${result.totalPoints}`, 'success');
                    await window.initSystemData(); 
...
document.getElementById('agentForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const totalFiles = (STATE.sceneFiles?.length || 0) + (STATE.objectFiles?.length || 0);
        if (totalFiles > 0) await window.addAgentLog('影像處理組', '📐', `偵測到 ${totalFiles} 張實境圖，進行特徵分析中...`, true);
        ...
        await window.addAgentLog('首席文案', '✍️', '正在與 Gemini 大腦連線撰寫腳本...', true);
...
window.submitForImageGeneration = async function() {
    ...
    window.scrollTo({ top: 0, behavior: 'smooth' });
    await window.addAgentLog('美術總監', '👨‍🎨', '收到發包指令！正在打包圖文參數...', true, btn);
    ...
    try {
        ...
        if(aiCount > 0) await window.addAgentLog('算圖農場', '🤖', `極速生成 ${aiCount} 張高畫質圖片中...`, true);
        else await window.addAgentLog('影像處理組', '☁️', '原圖安全上傳中...', true);
