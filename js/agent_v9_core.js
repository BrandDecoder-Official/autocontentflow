        ...
        <div class="space-y-4 lg:space-y-6 flex flex-col relative">
            <div class="bg-blue-600/10 p-4 lg:p-5 rounded-2xl border border-blue-500/30">
                <div class="grid grid-cols-2 gap-3">
                    <button ${isEnhance ? 'disabled' : `onclick="window.quickEdit('RATIO')"`} class="flex flex-col items-center justify-center bg-slate-800 p-3 rounded-xl border border-white/10 group ${isEnhance ? 'opacity-50' : ''}"><span class="text-[10px] text-slate-400 font-bold">⛭ 比例</span><span class="text-lg font-black text-white tag-ratio">${MISSION.ratio}</span></button>
                    <button onclick="window.quickEdit('RES')" class="flex flex-col items-center justify-center bg-slate-800 p-3 rounded-xl border border-white/10 group"><span class="text-[10px] text-slate-400 font-bold">⛭ 解析度</span><span class="text-lg font-black text-white tag-res">${MISSION.resolution}</span></button>
                </div>
            </div>
...
async function triggerCharacterPicker(container) {
    ...
    available.forEach(char => {
        ...
        const card = document.createElement('div'); card.className = `flex-shrink-0 flex flex-col items-center gap-1 cursor-pointer transition-all ${isSelected ? 'opacity-50 ring-2 ring-blue-500 rounded-full' : ''}`;
...
async function triggerMissionSummary() {
    ...
    const ui = createSkillUI(`
        <div class="bg-slate-900 border border-blue-500/30 rounded-2xl lg:rounded-3xl p-4 lg:p-6 shadow-2xl space-y-4 relative">
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center cursor-pointer p-2 rounded-lg hover:bg-white/5" onclick="window.retryStep('PLATFORM')"><span class="text-[11px] lg:text-xs text-slate-500">🚀 平台</span><span class="text-[11px] lg:text-xs font-bold text-white">${MISSION.platforms.join(', ')} ✎</span></div>
                <div class="flex justify-between items-center cursor-pointer p-2 rounded-lg hover:bg-white/5" onclick="window.retryStep('TOPIC')"><span class="text-[11px] lg:text-xs text-slate-500">📝 主題</span><span class="text-[11px] lg:text-xs font-bold text-white truncate max-w-[120px] lg:max-w-[200px]">${MISSION.topic} ✎</span></div>
                <div class="flex justify-between items-center cursor-pointer p-2 rounded-lg hover:bg-white/5" onclick="window.retryStep('UNIVERSE')"><span class="text-[11px] lg:text-xs text-slate-500">🌌 視覺</span><span class="text-[11px] lg:text-xs font-bold text-white">${uniMap[MISSION.universe]} / ${st ? st.name : ''} ✎</span></div>
                <div class="flex justify-between items-center cursor-pointer p-2 rounded-lg hover:bg-white/5" onclick="window.retryStep('VISUAL')"><span class="text-[11px] lg:text-xs text-slate-500">📐 參數</span><span class="text-[11px] lg:text-xs font-bold text-white">${MISSION.ratio} / ${MISSION.resolution} ✎</span></div>
                <div class="flex justify-between items-center cursor-pointer p-2 rounded-lg hover:bg-white/5" onclick="window.retryStep('VISUAL')"><span class="text-[11px] lg:text-xs text-slate-500">👥 素材</span><div class="flex items-center gap-2">${assetsHtml} <span class="text-xs font-bold text-white">✎</span></div></div>
            </div>
            ...
            <div class="pt-4 border-t border-white/10">
                ...
                <div class="flex gap-2">
                    <button id="btnOpenSch" class="flex-1 bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 border border-white/10 py-3 rounded-xl text-xs font-bold transition-colors">🕒 設定排程</button>
                    <button id="btnRender" class="flex-[2] bg-blue-600 hover:bg-blue-500 py-3 rounded-xl font-black text-xs lg:text-sm text-white shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-colors">⚡ 扣除 ${totalPts} 點並生成</button>
                </div>
            </div>
        </div>
    `);
...
function openScheduleModal(summaryUI) {
    ...
    panel.innerHTML = `
        ...
        <div class="space-y-4">
            ...
            <div class="flex gap-2 pt-2">
                <button id="btnSchNow" class="flex-1 bg-slate-700 text-white py-3 rounded-xl text-xs font-bold hover:bg-red-500/80 transition-colors">取消排程 (NOW)</button>
                <button id="btnConfirmSch" class="flex-1 bg-indigo-600 text-white py-3 rounded-xl text-xs font-bold shadow-lg shadow-indigo-900/30 hover:bg-indigo-500 transition-colors">✅ 確認寫入</button>
            </div>
        </div>
    `;
    ...
    panel.querySelector('#btnConfirmSch').onclick = () => {
        ...
        if (!selectedDate || !selectedTime) return showError("請完整選擇日期與時間");
        
        MISSION.scheduleMode = 'LATER'; MISSION.scheduleDate = selectedDate; MISSION.scheduleTime = selectedTime;
        summaryUI.querySelector('#schDisplay').innerText = `預約: ${selectedDate} ${selectedTime}`;
        summaryUI.querySelector('#schDisplay').classList.replace('text-slate-400', 'text-green-400');
        summaryUI.querySelector('#btnRender').innerText = `⚡ 扣點算圖，並加入排程佇列`;
        window.closeScheduleModal();
    };

    panel.querySelector('#btnSchNow').onclick = () => {
        MISSION.scheduleMode = 'NOW'; MISSION.scheduleDate = ''; MISSION.scheduleTime = '';
        summaryUI.querySelector('#schDisplay').innerText = `立即發布 (NOW)`;
        summaryUI.querySelector('#schDisplay').classList.replace('text-green-400', 'text-slate-400');
        summaryUI.querySelector('#btnRender').innerText = `⚡ 扣點並立即生成`;
        window.closeScheduleModal();
    };
}
