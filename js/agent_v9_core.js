...
// 🌟 版本控制號
const APP_VERSION = "V0.7版";

// 🚀 任務卷宗
const MISSION = {
    platforms: [],
    topic: '',
    universe: 'REALISTIC', // REALISTIC | COMIC | ENHANCE
    style: '',             
    ratio: '9:16',
    resolution: '1K',
    characters: [],
...
export async function initAgentFunnel() {
    ...
    await addLog("專案總監", "👨‍💼", `${APP_VERSION} - 總編您好，「無損美化專線」與「側欄基因庫連動」已部署完畢。請下達指令。`);
...
async function triggerUniverseSkill() {
    ...
    ui.querySelectorAll('.uni-btn').forEach(btn => {
        btn.onclick = async () => {
            ...
            // 🤖 代理人智能洗牌：切換到美化模式時，清空角色並鎖定比例
            if (MISSION.universe === 'ENHANCE') {
                MISSION.characters = [];
                MISSION.ratio = '原圖比例';
            } else if (oldUniverse === 'ENHANCE') {
                MISSION.ratio = '9:16'; // 切回來時恢復預設
            }
...
/** 🛠️ Skill 5: 視覺參數 (🌟 V0.7: 根據宇宙智能鎖定按鈕) */
async function triggerVisualSkill() {
    updateStepHeader("VISUAL CONFIG");
    const isEnhance = MISSION.universe === 'ENHANCE';
    
    let promptMsg = isEnhance 
        ? "偵測為美化模式。請【務必上傳原圖】，角色召喚與比例調整已自動鎖定："
        : "最後，請確認畫面參數，並【務必召喚至少一位角色】：";
    ...
    ui.querySelector('#btnAcceptVisual').onclick = async () => {
        // 🛡️ 智能防呆：針對不同模式有不同要求
        if (isEnhance) {
            if (MISSION.sceneFiles.length === 0) return alert('⚠️ 美化模式防呆：請點擊右下角上傳需要美化的原圖！');
        } else {
            if (MISSION.characters.length === 0) return alert('⚠️ 系統防呆：您尚未召喚任何角色！請點擊「🧬 召喚角色基因」。');
        }
...
// 模擬腳本生成與審閱 (省略部分重複代碼，保持與 V0.6 一致)
...
async function triggerDraftReviewSkill(draft) {
    ...
    await addLog("專案總監", "👨‍💼", "總編，腳本已出爐！您可以檢視與修改：", true);
    ...
    const ui = createSkillUI(`
        <div class="bg-slate-900 border border-indigo-500/30 rounded-2xl p-5 shadow-2xl space-y-4">
            ...
            <div class="bg-indigo-900/30 p-3 rounded-xl border border-indigo-500/40">
                <p class="text-[10px] text-indigo-300 font-black mb-2 uppercase">👗 依主題推薦之外觀裝扮</p>
                <textarea class="w-full bg-slate-800/50 border border-white/5 rounded-lg p-2 text-xs text-white resize-none" rows="2">依照「${MISSION.topic}」搭配</textarea>
            </div>` : ''}
            <div>
            ...
            <button id="btnRender" class="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-black text-sm shadow-lg shadow-green-900/30">🎨 確認無誤，扣點算圖</button>
        </div>
    `);

    ui.querySelector('#btnRender').onclick = async () => {
        lockUI(ui); await addLog("美術總監", "🎨", "腳本定案！正式啟動視覺引擎...", true);
    };
}
...
async function triggerCharacterPicker() {
    ...
    manageBtn.innerHTML = '<span class="text-xl">⚙️</span>';
    manageBtn.onclick = () => { if(window.openCharacterLib) window.openCharacterLib(); };
    charDiv.appendChild(manageBtn);
